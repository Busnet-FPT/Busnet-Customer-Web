...
function getPaymentStatus(booking: BookingHistoryItem) {
...
function MyBookingsPage() {
  ...
  const handleCancelBooking = async (bookingCode: string) => {
    ...
    try {
      const response = await cancelBooking(bookingCode)
      toast.success(response.data?.message || response.message || 'Booking cancelled successfully!', { id: cancelToast })
      loadHistory()
    } catch (err) {
      console.error(err)
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    if (activeTab === 'PENDING') {
      return paymentStatus === 'PENDING' && status !== 'CANCELLED_BY_CUSTOMER' && status !== 'CANCELLED_BY_OPERATOR'
    }
    if (activeTab === 'PAID') {
      return paymentStatus === 'PAID' || paymentStatus === 'SUCCESS' || status === 'CONFIRMED' || status === 'COMPLETED'
    }
    if (activeTab === 'CANCELLED') {
      return status === 'CANCELLED_BY_CUSTOMER' || status === 'CANCELLED_BY_OPERATOR' || paymentStatus === 'CANCELLED'
    }
  ...
  const renderStatusBadges = (b: BookingHistoryItem) => {
    ...
    if (status === 'PENDING_PAYMENT') {
      ...
      statusText = paymentStatus === 'EXPIRED' ? 'Expired' : 'Cancelled'
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {filteredBookings.map((b) => {
              const anyB = b as any
              const tripData = anyB.tripId || anyB.trip || {}
              const routeData = tripData.routeId || tripData.route || {}
              const busData = tripData.busId || tripData.bus || {}
              const routeName = routeData.routeName || anyB.trip?.route?.routeName || 'Unnamed Route'
              ...
              const departureTime = tripData.departureTime || formatMinutes(tripData.actualDepartureTime)
              const arrivalTime = tripData.arrivalTime || formatMinutes(tripData.actualArrivalTime)
              const pickupName = anyB.pickupPoint_name || anyB.pickupPoint?.name || ''
              const pickupTime = anyB.pickupPoint_time || anyB.pickupPoint?.time || ''
              const dropoffName = anyB.dropoffPoint_name || anyB.dropoffPoint?.name || ''
              const dropoffTime = anyB.dropoffPoint_time || anyB.dropoffPoint?.time || ''
              const paymentStatus = getPaymentStatus(b)
              const isPending = paymentStatus === 'PENDING' && b.status !== 'CANCELLED_BY_CUSTOMER' && b.status !== 'CANCELLED_BY_OPERATOR'
              const isPaid = paymentStatus === 'PAID' || paymentStatus === 'SUCCESS' || b.status === 'CONFIRMED' || b.status === 'COMPLETED'

              return (
                <div
                  key={b.bookingId || b._id || b.bookingCode}
                  onClick={() => {
                    if (isPending) navigate(`/payment/${b.bookingCode}`)
                    else if (isPaid) navigate(`/my-bookings/${b.bookingCode}`)
                  }}
                ...
                >
                  <div className="flex-1 space-y-4 flex flex-col justify-between min-w-0">
                    <div className="space-y-1">
                      <div className="flex items-center gap-3 flex-wrap">
                        <span className={`text-xs font-black text-slate-800 font-primary transition-colors ${isPending || isPaid ? 'group-hover:text-primary' : ''}`}>
                          {routeName}
                        </span>
                        ...
                        {busData.busName && (
                          <span className="text-[10px] font-extrabold text-slate-450 bg-slate-100 border border-slate-200/50 px-2 py-0.5 rounded">
                            {busData.busName}{busData.licensePlate ? ` - ${busData.licensePlate}` : ''}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 rounded-2xl bg-slate-50/70 border border-slate-100 p-3 text-[11px] text-slate-500 font-semibold">
                      <div>
                        <span className="block text-[9px] font-extrabold uppercase tracking-wider text-slate-400 font-primary">Departure</span>
                        <span className="text-slate-800 font-bold">{formatDate(departureDate)} - {departureTime}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] font-extrabold uppercase tracking-wider text-slate-400 font-primary">Arrival</span>
                        <span className="text-slate-800 font-bold">{arrivalTime}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] font-extrabold uppercase tracking-wider text-slate-400 font-primary">Pickup</span>
                        <span className="text-slate-800 font-bold">{pickupName || routeData.originProvince || routeData.origin_provinceName || 'N/A'}{pickupTime ? ` - ${pickupTime}` : ''}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] font-extrabold uppercase tracking-wider text-slate-400 font-primary">Dropoff</span>
                        <span className="text-slate-800 font-bold">{dropoffName || routeData.destinationProvince || routeData.destination_provinceName || 'N/A'}{dropoffTime ? ` - ${dropoffTime}` : ''}</span>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3 text-[11px] text-slate-500 font-semibold border-t border-slate-100/60 pt-3">
                      <div>
                        <span className="block text-[9px] font-extrabold uppercase tracking-wider text-slate-400 font-primary">Passenger</span>
                        <span className="block text-slate-800 font-bold">{b.passengerName || '—'}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] font-extrabold uppercase tracking-wider text-slate-400 font-primary">Phone</span>
                        <span className="block text-slate-800 font-bold">{b.passengerPhone || '—'}</span>
                      </div>
                    </div>
                  </div>
